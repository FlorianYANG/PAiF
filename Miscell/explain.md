...
And if you go further to the math, you will see that when vol goes up, Delta is no more N(d2) (It never is at initially, N(d2) is just an approximation). That is, Delta is not the probability that stock price will be within the strike or option being execute. When vol goes to that level everything changes like when you are at light speed, time will stop.
